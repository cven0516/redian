...
给请不起营销团队的小店用的热点活动营销助手。只要告诉它你的店在哪、卖什么、有什么困境，10分钟内帮你：
...
大品牌花几万做一个借势活动，你用这个工具 10 分钟就能出一个同等水准的策划——**热点发现 → 策划匹配 → SMART 方案 → 视觉内容矩阵 → Dashboard 看板，全链路闭环。**

![Dashboard 预览](assets/dashboard-preview.jpg)
...
- **视觉有实物** — 品牌资产收集 → 全链路触点规划 → 提示词 → Concept Board → 精确生图
...
```
用户输入 → 门店快诊 → 热点雷达 → 方向匹配(用户选) → 策划匹配(用户选) → SMART策划案 → [视觉规划] → [Dashboard]
```
...
| **HTML Dashboard** | 可交互策划看板（呼吸感动效、Tab切换、响应式） |
